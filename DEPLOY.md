...
目标：在服务器上以 `http://<服务器IP>:8001` 对外提供服务（下文 `<服务器IP>` 替换成你的实际服务器地址）。
...
| 3 | **改默认密码** | 登录 `admin/123456` 后，到「系统管理 → 用户管理」修改 **admin** 密码 |
| 4 | **放行端口** | 服务器防火墙/安全组放行 **8001** 入站 |
| 5 | **确认基础数据** | `SEED_DEMO_DATA=false` 时首次启动只创建管理员、两个基础机房、基础设备和巡检模板，不创建演示巡检记录 |
| 6 | **建好角色账号** | 闭环需要：至少 1 个**巡检员**、1 个**处理员**、1 个**核实员**，请在用户管理里按实际人员新增 |
...
| 变量 | 说明 |
|---|---|
| `PUBLIC_WEB_BASE_URL` | 机房二维码里编码的地址。**必须是手机能访问到的地址**（如 `http://<服务器IP>:8001`）。若填 localhost，二维码会被标记为「不可打印」。 |
